...
- [X] T025 [P] [US2] Test named-pipe happy path: connect over `\\.\pipe\pglite_example`, assert `SELECT 1` works and the bridge accept log records `transport=pipe`; assert no process bound 54320 during the run (via a helper that calls `Get-NetTCPConnection` or `netstat`) in `tests/windows_sample_db/test_transport_pipe.py`
- [X] T026 [US2] Test `--unique-pipe` and stable-name collision: two concurrent runs with default name → second exits 5 naming the pipe; same two with `--unique-pipe` → both succeed on distinct pipe names (FR-025, FR-026) in `tests/windows_sample_db/test_transport_pipe.py`

### Implementation for User Story 2

- [X] T027 [US2] Implement the bridge's `--transport pipe` branch: `net.createServer().listen('\\\\.\\pipe\\<name>')` — Node's default pipe ACL on Windows is creator+SYSTEM only, which satisfies the "no Everyone ACE" requirement in `contracts/transport.md`. The bridge process does not bind any TCP port on this branch (verified by T025). Emits `[bridge] start transport=pipe listen=\\\\.\\pipe\\<name> data_dir=<abs>`. File: `examples/windows_sample_db/node/pglite_bridge.js`
- [X] T028 [US2] psycopg 3.3.3 does NOT expose a pre-connected-stream factory (verified at implementation time — `PGconn` only exposes `connect`/`connect_start`; no hook to consume an already-open handle). Research R2's optimistic path was unavailable, so this task collapsed into detection logic; the actual Python adapter flows through T029 in every run. File: `examples/windows_sample_db/transport.py`
- [X] T029 [US2] Implemented `PipeRelay` in `examples/windows_sample_db/transport.py` — bind `127.0.0.1:0`, accept psycopg's socket, open a fresh `CreateFile(...,FILE_FLAG_OVERLAPPED,...)` pipe handle per connection, splice bytes with two overlapped-I/O threads. Overlapped was required: default sync pipe I/O serializes concurrent `ReadFile`/`WriteFile` on the same handle and deadlocks bidirectional Postgres traffic.
- [X] T030 [US2] Wired `--transport pipe`, `--pipe-name`, `--unique-pipe` in argparse (already seeded in T015). `TransportConfig._resolved_pipe_name` is cached in `__post_init__` so the launcher and bridge see the same `pglite_example_<pid>_<uuid4[:8]>` string. Env fallback `PGLITE_EXAMPLE_TRANSPORT` is consulted via `argparse.default=` when `--transport` is absent. File: `examples/windows_sample_db/run_example.py`
- [X] T031 [US2] Pipe-collision detection in launcher: pre-flight via `win32pipe.WaitNamedPipe(path, 0)`, plus `proc.poll() == 5` branch mapping bridge EADDRINUSE to `PortInUseError`. Message names the pipe and suggests `--unique-pipe` (FR-026). File: `examples/windows_sample_db/launcher.py`
- [X] T031b [US2] Pipe-unavailable detection: `proc.poll() == 4` branch in the readiness loop raises `BridgePipeDenied` (exit 4) with an FR-010-shaped message that names the pipe path and recommends `--transport tcp`. Bridge side maps `EACCES`/`EPERM` listen errors to exit 4. File: `examples/windows_sample_db/launcher.py`
- [X] T032 [US2] `run_example.py` builds a `TransportConfig` that carries the requested `kind` through, and dials psycopg via `handle.dsn()` which picks the TCP DSN or the relay-port DSN depending on transport. `invoke_all_procedures()` is transport-agnostic and runs identically over TCP and pipe.
