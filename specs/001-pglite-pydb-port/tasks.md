...
- [ ] T005 Mass-rewrite Python imports across `src/pglite_pydb/`, `tests/`, `examples/`, `conftest.py`: `from py_pglite` → `from pglite_pydb`, `import py_pglite` → `import pglite_pydb`, whole-word `py_pglite.` → `pglite_pydb.`; verify with `rg -nw "py_pglite" --glob "*.py"` returning 0 hits and the Linux pytest suite equalling the T001 baseline (Step 4)
...
- [ ] T010 [US1] Modify `PGliteConfig` in `src/pglite_pydb/config.py` to distinguish user-set vs default `use_tcp` (add a private sentinel or `_use_tcp_explicit` flag set by `__init__` override) so `__post_init__` can apply the platform-conditional promotion without breaking fields preservation from contracts/public-api.md Contract 3
...
- [ ] T017 [US1] Windows spike verification: on a Windows 11 host run `uv run python -c "from pglite_pydb import PGliteConfig; c = PGliteConfig(); print(c.use_tcp, c.tcp_port)"` → expected output `True 0`; then run a minimal `SELECT 1` pytest and confirm `Get-Process node` after the run shows no survivors (SC-003 spot-check)
...
- [ ] T023 [US2] Smoke-test the SQLAlchemy example at `examples/sqlalchemy-example/` (or whichever example exists in the repo) with only the import lines rewritten from `py_pglite` to `pglite_pydb`; confirm it runs end-to-end without any other source edit (SC-007)
- [ ] T024 [US2] Smoke-test the Django example at `examples/django-example/` (or whichever Django example exists) with the `ENGINE` string in the test `DATABASES` config updated from `"py_pglite.django.backend"` to `"pglite_pydb.django.backend"` and imports rewritten; confirm it passes unmodified otherwise
...
- [ ] T030 [US3] Sweep `tests/` for tests that legitimately require a Unix domain socket (tests that construct socket paths directly, tests that pass `use_tcp=False` unconditionally, etc.) and add `@pytest.mark.skipif(IS_WINDOWS, reason="Unix socket transport")` using `from pglite_pydb._platform import IS_WINDOWS`; tag each test's file and line in the commit message for review (FR-012, Step 11 part 5)
...
- [ ] T032 [P] [US4] Create `tests/test_public_api_contract.py` with the six runnable test functions from [contracts/public-api.md](./contracts/public-api.md) §8: `test_top_level_imports`, `test_version_metadata_aligned`, `test_pytest_plugin_registered`, `test_config_fields_preserved`, `test_windows_rejects_explicit_unix_socket` (guarded with `skipif(not IS_WINDOWS)`), `test_legacy_import_fails`
...
- The 12-step ledger in [data-model.md](./data-model.md) Entity 7 maps to tasks as: Step 1 → T001; Step 2 → T003; Step 3 → T004; Step 4 → T005; Step 5 → T033/T034/T035; Step 6 → T006; Step 7 → T010/T011/T012/T013; Step 8 → T007/T008/T009; Step 9 → T014/T015/T016; Step 10 → T038/T039/T040/T041; Step 11 → T025/T026/T027/T028/T029/T030; Step 12 → T044–T051
