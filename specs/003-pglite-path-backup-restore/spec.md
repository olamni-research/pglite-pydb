...
- **Backup Container**: A self-describing single-file archive (`.tar.gz`) produced by `backup` inside the instance's configured Backup Location. Named by a chronologically-sortable timestamp. Internally contains one logical SQL dump file per included schema plus a `manifest.json` (timestamp, source instance path, included schemas) sufficient to validate a subsequent restore and to support `--latest` selection.
